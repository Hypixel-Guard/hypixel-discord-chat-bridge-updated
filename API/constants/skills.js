...
function getSkillAverage(profileData, hypixelPlayer, options = { decimals: 2, progress: false, cosmetic: false }) {
  ...
  for (const skillId of skillTables.skills) {
    ...
    const skillLevel = options.progress ? skill.levelWithProgress : skill.level;
    // @ts-ignore
    totalLevel += Math.min(skillLevel, skillTables.maxLevels[skillId] ?? skillLevel);
  }
...
function getOverflowLevel(xp) {
  ...
  while (true) {
    const n = level + 1 - 60;
    const segment = Math.ceil(n / 10);
    const isDecadeCrossing = (n - 1) % 10 === 0 && segment > 1;
    const usedDelta = 600000 * 2 ** (isDecadeCrossing ? segment - 2 : segment - 1);

    const nextCost = cost + usedDelta;
    const nextCumulative = cumulative + nextCost;

    if (nextCumulative > xp) {
      const xpCurrent = xp - cumulative;

      return {
        overflowLevel: level - 60,
        level,
        levelWithProgress: level + xpCurrent / nextCost,
        xpCurrent,
        xpForNext: nextCost,
        progress: xpCurrent / nextCost
      };
    }
